...
test('arquivo único inclui a versão clínica revisada', () => {
  assert.match(standalone, /guildbook-version" content="Versão 6\.3 · auditoria clínica e receituários expandidos"/);
...
test('standalone V6.3 contém a expansão líquida de 150 princípios ativos', () => {
  const { MEDICINES, normalizeActiveName } = standalonePrescriptionApi();
  assert.ok(MEDICINES.length >= 322);
  assert.equal(new Set(MEDICINES.map(medicine => medicine.id)).size, MEDICINES.length);
  assert.equal(new Set(MEDICINES.map(medicine => normalizeActiveName(medicine.name))).size, MEDICINES.length);
});
